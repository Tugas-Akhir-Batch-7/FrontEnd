    ...
    <table class="table table-hover" style="margin-bottom:2em">
      ...
      <tbody>
        <tr v-for="(value, key) in listTugas" :class="{'table-danger':editMode && listDeleteTugas.includes(value.id)}">
          <td style="width:5%; text-align:center">{{key+1}}</td>
          <!--name-->
          <td class="w-25" v-if="editMode"><input v-model="listTugas[key].name" @keyup="editTugas(value.id, listTugas[key].name, 'name', key)" class="form-control form-control-sm" type="text"></td>
          <td class="w-25" v-else>{{value.name}}</td>
          <!--description-->
          <td v-if="editMode">
            <input :class="{'d-none':!listEditTugas[value.id],'d-block':listEditTugas[value.id]}" v-model="listEditTugas[value.id].description" @keyup="editTugas(value.id, listTugas[key].description, 'desc', key)" ref="barbar" class="form-control form-control-sm" type="text">
            <input :class="{'d-none':listEditTugas[value.id],'d-block':!listEditTugas[value.id]}" v-model="listTugas[key].description" @focus="editTugas(value.id, listTugas[key].description, 'desc', key)" class="form-control form-control-sm" type="text">
          </td>
...
export default {
  ...
  methods:{
    ...
    async editTugas(id, ket, e, i, event){
      try{
        console.log(this.listEditTugas[id])
        if(!this.listEditTugas[id]){
          this.listEditTugas[id] = {}
          this.listEditTugas[id].name = this.listTugas[i].name
          this.listEditTugas[id].description = this.listTugas[i].description
        }
        console.log(this.listEditTugas[id])
        // if(e == 'desc') this.listEditTugas[id].description = ket
        // else if(e == 'name') this.listEditTugas[id].name = ket
        // console.log(id)
        // console.log(ket)
        // console.log(e)
        // event.target.select();
